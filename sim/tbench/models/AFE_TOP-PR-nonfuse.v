...
module AFE_TOP ( ADCOUT_LSB, ADCOUT_MSB, CK250K, CV_DONE,
     ...
     //tcwang MODIFY 20260203 PR-nonfuse
     from_dig_A,to_dig_D,from_dig_AEN,from_dig_RDEN,from_dig_PGMEN,dvddenb_fuse,
     ///
...
//fuse
input [7:0] from_dig_A;
output [7:0] to_dig_D;
input from_dig_AEN,from_dig_RDEN,from_dig_PGMEN;
input dvddenb_fuse;
//to digital
...
wire [7:0] from_dig_A,to_dig_Din,to_dig_D;
wire from_dig_AEN,from_dig_RDEN,from_dig_PGMEN;

assign to_dig_D = to_dig_Din&{8{!dvddenb_fuse}};


 `ifdef FPGA
assign to_dig_D=0;
`else
	always @(POR or dvddenb_fuse)  
	if(!POR) DVDD<=0;
	else if(dvddenb_fuse) DVDD<=0;
	else if(!dvddenb_fuse) #50000 DVDD<=1 ;
	else DVDD<=0;

S0153GEFUSE_PIPO256B xS0153GEFUSE_PIPO256B (
	.AEN(from_dig_AEN),
	.RDEN(from_dig_RDEN),
	.PGMEN(from_dig_PGMEN),
	.A(from_dig_A),
	.D(to_dig_Din),
	.DVDD(DVDD),
	.AVDD(avdd_fuse),
